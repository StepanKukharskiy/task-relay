...
def required(spec):return spec.get('execution',{}).get('capability')==CAPABILITY
...
def binding(rt,spec):
    ...
    inputs=[]
    fields={'application/x-blender':'scene_sha256','text/x-python':'script_sha256','application/json':'checks_sha256'}
    for item in spec['inputs']:
        ...
        if item['media_type']=='application/json':validate_checks(json.loads(Path(artifact['blob']).read_text()))
    ...
    return {'assignment_digest':c.digest(spec),'assignment':spec,'inputs':inputs,'application_signature':application_signature(app['executable']),
            'runtime_sources':{n:file_hash(Path(__file__).with_name(n)) for n in SOURCES},
...
def approved(rt,run,tid,spec):
    ...
    if not row:raise ValueError('Exact-script host approval is required before Blender Python can launch')
    value=json.loads(row['data'])
    if value['binding']!=binding(rt,spec):raise ValueError('Script, inputs, assignment, Blender or implementation changed after host approval')
    return value
...
def verify_frozen(frozen):
    ...
    if grant['application_signature']!=application_signature(blender()['executable']):raise ValueError('Blender changed after approval')
    if grant['runtime_sources']!={n:file_hash(Path(__file__).with_name(n)) for n in SOURCES}:raise ValueError('Host-code implementation changed after approval')
